...
--
local Intensity = 18; --[[      CHANGE YOUR INTENSITY CAMERA MOVEMENT HERE       ]]
-- default : 40
...
function onUpdatePost()
   if followchars and startCam then
      ...
      if not stringEndsWith(anim, 'miss') then
         if stringStartsWith(anim, "singLEFT") then
            camOffY = 0;
            camOffX = 0 - Intensity; -- this
         end
         if stringStartsWith(anim, "singRIGHT") then
            camOffY = 0;
            camOffX = Intensity;
         end
         if stringStartsWith(anim, "singUP") then
            camOffX = 0;
            camOffY = 0 - Intensity;
         end
         if stringStartsWith(anim, "singDOWN") then
            camOffX = 0;
            camOffY = Intensity;
         end
         if stringStartsWith(anim, "idle") then
            camOffX = 0;
            camOffY = 0;
         end
      end
